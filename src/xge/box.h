...
public:
	...
	*/
	...
	{
		return 
			   !isnan(this->p1.x) && isfinite(this->p1.x)
			&& !isnan(this->p1.y) && isfinite(this->p1.y)
			&& !isnan(this->p1.z) && isfinite(this->p1.z)
			&& !isnan(this->p2.x) && isfinite(this->p2.x)
			&& !isnan(this->p2.y) && isfinite(this->p2.y)
			&& !isnan(this->p2.z) && isfinite(this->p2.z)
			&& this->p1.x<=this->p2.x 
			&& this->p1.y<=this->p2.y 
			&& this->p1.z<=this->p2.z;
	}
...
public:
	...
	*/
	inline bool isValid() const
	{
		...
		{
			if (isnan(p1[i]) || !isfinite(p1[i])) return false;
