...
class ByteSwap 
{
	union {T word;unsigned char bytes[sizeof(T)];} _data;

	inline void swapBytes() 
	{
		for(unsigned char tmp,k = 0,n = sizeof(T)-1;k < n; k++,n--) 
		{
			tmp = _data.bytes[k];
			_data.bytes[k] = _data.bytes[n];
			_data.bytes[n] = tmp;
		}
	}
...
public:
	...
	inline ByteSwap() {}
	inline ByteSwap(T const &w) {_data.word = w;swapBytes();}
	inline T &value() {return _data.word;}
	inline T &operator=( T const &w )  {_data.word = w;swapBytes();return _data.word;}
};
...
//---------------------------------------------------------
//---------------------------------------------------------
std::vector<SmartPointer<Batch> > Batch::openPly(std::string filename,bool bReverseNormals)
{
	static const union 
	{
		unsigned short u;
	...
	if (!ply || ply->file_type != PLY_ASCII)
	{
		if (ply) close_ply( ply );
		file = fopen( filename.c_str(), "rb" );
		ply  = read_ply(file);
		if (!ply) return std::vector<SmartPointer<Batch> >();
	}
	
	bool do_swap = false;
	if( ply->file_type == PLY_ASCII ) 
	{
		Log::printf("Reading PLY ascii file, version \n");
		do_swap=false;
	} 
	else if( ply->file_type == PLY_BINARY_BE ) 
	{
		Log::printf("Reading PLY binary file (big endian), version \n");
		do_swap = endian.is.little>0;
	}
	else if( ply->file_type == PLY_BINARY_LE ) 
	{
		Log::printf("Reading PLY binary file (little endian), version  \n");
		do_swap = endian.is.big>0;
	}
	
	int    ncomments;
	char **comments = get_comments_ply( ply, &ncomments );

	int    nobjs;
	char **obj_info = get_obj_info_ply( ply, &nobjs );
	...
	int npoints;
	int nprops;
	PlyProperty** props= get_element_description_ply( ply, (char*)"vertex", &npoints, &nprops);
	...
	bool bHasNormals;
	{
		...
		{
			     if (!strcmp(props[k]->name,"nx")) cont++;
			else if (!strcmp(props[k]->name,"ny")) cont++;
			else if (!strcmp(props[k]->name,"nz")) cont++;
		}
		bHasNormals=(cont==3);
	}
	...
	struct PlyVertex {float x,y,z,nx,ny,nz;};
	...
	get_element_setup_ply( ply, (char*)"vertex", bHasNormals?6:3, vert_props);

	for (int k=0; k<npoints; ++k) 
	{
		PlyVertex V={0,0,0,0,0,0};
		get_element_ply(ply,(void*)&V);
		
		if( do_swap )
		{
			V.x=ByteSwap<float>()=V.x;
			V.y=ByteSwap<float>()=V.y;
			V.z=ByteSwap<float>()=V.z;

			if (bHasNormals)
			{
				V.nx=ByteSwap<float>()=V.nx;
				V.ny=ByteSwap<float>()=V.ny;
				V.nz=ByteSwap<float>()=V.nz;
			}
		}
		...
		if (bHasNormals)
		{
			Vec3f n=Vec3f(V.nx,V.ny,V.nz).normalize();
			if (bReverseNormals) n=n*-1;
	...
	{	
		...
		PlyProperty face_prop = {(char*)"vertex_indices", Int32, Int32, offsetof(PlyFace,verts),PLY_LIST, Uint8, Uint32, offsetof(PlyFace,nverts)};
		get_element_setup_ply( ply, (char*)"face", 1, &face_prop );

		for(int k=0; k<nfaces; ++k ) 
		{
			PlyFace face;
			get_element_ply( ply,(void*)(&face) );
			for( int j=0; do_swap && j<(int)face.nverts; ++j ) face.verts[j] = ByteSwap<int>() = face.verts[j];
			
			for( int j=2; j<(int)face.nverts; ++j ) 
			{
				if (false)
				{
					triangles.push_back(face.verts[j  ]);
					triangles.push_back(face.verts[j-1]);
	...
	else if (props = get_element_description_ply( ply, (char*)"tristrips", &nstrips, &nprops))
	{
		...
		PlyProperty trianglestrip_prop = {(char*)"vertex_indices", Int32, Int32, offsetof(PlyTriangleStrip,verts),PLY_LIST, Uint8, Uint32, offsetof(PlyTriangleStrip,nverts)};
		get_element_setup_ply( ply, (char*)"tristrips", 1, &trianglestrip_prop );

		for(int k=0; k<nstrips; ++k ) 
		{
			PlyTriangleStrip strip;
			get_element_ply( ply, (void*)(&strip) );
			for( int j=0; do_swap && j<(int)strip.nverts; ++j ) strip.verts[j] = ByteSwap<int>() = strip.verts[j];

			bool clockwise = true;
			for (int I=2; I < (int)strip.nverts; I++) 
			{
				if (strip.verts[I] == -1) 
				{
					I += 2;
				...
				{
					triangles.push_back(clockwise?strip.verts[I-2]:strip.verts[I-2]);
					triangles.push_back(clockwise?strip.verts[I-1]:strip.verts[I  ]);
					triangles.push_back(clockwise?strip.verts[I  ]:strip.verts[I-1]);					
					clockwise = !clockwise;
				}
			}
		}
	...
	//if index start from 1 correct it
	#if 1
	bool ply_vertex_one_ref=true;
	for( int k=0 ; ply_vertex_one_ref && k<(int)triangles.size(); k++ ) ply_vertex_one_ref=(triangles[k]>0);
	for( int k=0 ; ply_vertex_one_ref && k<(int)triangles.size(); k++ ) --triangles[k];
	#endif
	...
	{
		//automatic calculation of normals per face
		normals.clear();
