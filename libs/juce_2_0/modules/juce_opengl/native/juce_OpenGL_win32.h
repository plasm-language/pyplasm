...
private:
    ...
    static void initialisePixelFormatDescriptor (PIXELFORMATDESCRIPTOR& pfd, const OpenGLPixelFormat& pixelFormat)
    {
        ...
        //visus (if VISUS_GL_SWAP_COPY_PIXELS==1 then swapBuffers is never called, so this is a NOP)
        {
        	...
        	PFD_SWAP_COPY is a hint only and might not be provided by a drive */
        	printf            ("VISUS ChoosePixelFormat(+PFD_SWAP_COPY) on Windows platform\n");
        	OutputDebugStringA("VISUS ChoosePixelFormat(+PFD_SWAP_COPY) on Windows platform\n");
        	pfd.dwFlags |= PFD_SWAP_COPY;

#if VISUS_ZSPACE
          printf            ("VISUS ChoosePixelFormat(+PFD_STEREO) on Windows platform (VISUS_ZSPACE enabled)\n");
          OutputDebugStringA("VISUS ChoosePixelFormat(+PFD_STEREO) on Windows platform (VISUS_ZSPACE enabled)\n");
    ...
    int wglChoosePixelFormatExtension (const OpenGLPixelFormat& pixelFormat) const
    {
        ...
        if (wglChoosePixelFormatARB != nullptr)
        {
            ...
            //visus (if VISUS_GL_SWAP_COPY_PIXELS==1 then swapBuffers is never called, so this is a NOP)
            {
            	printf            ("VISUS WGL_SWAP_METHOD_ARB=WGL_SWAP_COPY_ARB on Windows platform\n");
            	OutputDebugStringA("VISUS WGL_SWAP_METHOD_ARB=WGL_SWAP_COPY_ARB on Windows platform\n");
            	#define WGL_SWAP_COPY_ARB 0x2029
            	atts[n++] = WGL_SWAP_METHOD_ARB; 
            	atts[n++] = WGL_SWAP_COPY_ARB; 

#if VISUS_ZSPACE
            	printf            ("VISUS WGL_STEREO_ARB on Windows platform (VISUS_ZSPACE enabled)\n");
            	OutputDebugStringA("VISUS WGL_STEREO_ARB on Windows platform (VISUS_ZSPACE enabled)\n");
